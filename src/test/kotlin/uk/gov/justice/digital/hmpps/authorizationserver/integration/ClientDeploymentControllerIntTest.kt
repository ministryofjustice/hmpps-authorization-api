...
class ClientDeploymentControllerIntTest : IntegrationTestBase() {
  ...
  @Nested
  inner class GetClientDeployment {
    @Test
    fun `get client deployment success`() {
      webTestClient.post().uri("/clients/testz/deployment")
        .headers(setAuthorisation(roles = listOf("ROLE_OAUTH_ADMIN")))
        .body(
          BodyInserters.fromValue(
            mapOf(
        ...
        .exchange()
        .expectStatus().isOk
      webTestClient.get().uri("/clients/testz/deployment")
        .headers(setAuthorisation(roles = listOf("ROLE_OAUTH_ADMIN")))
        .exchange()
        .expectStatus().isOk
        .expectBody().json(
          """
               {
                "clientType": "PERSONAL",
                "team": "testing team",
                "teamContact": "testz lead",
                "teamSlack": "#testz",
                "hosting": "CLOUDPLATFORM",
                "namespace": "testy-testing-dev",
                "deployment": "hmpps-testing-dev",
                "secretName": "hmpps-testing",
                "clientIdKey": "SYSTEM_CLIENT_ID",
      ...
      verify(telemetryClient).trackEvent(
        "GetAuthorizationServerClientDeploymentDetails",
        mapOf("username" to "AUTH_ADM", "baseClientId" to "testz"),
        null,
      )
    }
    ...
    @Test
    fun `access forbidden when no authority`() {
      webTestClient.get().uri("/clients/testy/deployment")
        .exchange()
        .expectStatus().isForbidden
    }
    ...
    @Test
    fun `unrecognised client id`() {
      webTestClient.get().uri("/clients/testy_unknown/deployment")
        .headers(setAuthorisation(roles = listOf("ROLE_OAUTH_ADMIN")))
        .exchange()
        .expectStatus().isNotFound
        .expectBody()
